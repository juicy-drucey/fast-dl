...
function SWEP.MainBullet:Penetrate(ply, traceres, dmginfo, weapon, penetrated, previousStartPos)
	...
	local enttype = type(hitent)
	
	if(bshields and enttype == "Entity") then
		local entclass = hitent:GetClass()
		if(entclass == "bs_dshield" or entclass == "bs_hshield" or entclass == "bs_shield" or (entclass == "bs_rshield" and bshields.config.rshieldbp)) then
			return 
		end
	end
