...
function SWEP:CalcViewModelView(vm, oldEyePos, oldEyeAngles, eyePos, eyeAngles)
	...
	local angAdd = angle_zero
	if STUNGUN.CanHolster then
		if IsFirstTimePredicted() and self:IsHolstered() != self.LastHolsteredState then
			self.LastHolsteredState = self:IsHolstered()
			self.HolsterAnimStart = RealTime()
		end
		
		local progress
		if self.HolsterAnimStart and (self.HolsterAnimStart + self.HolsterTime) > RealTime() then
			progress = easing((RealTime() - self.HolsterAnimStart) / self.HolsterTime)
		else
			progress = 1
		end

		if self.LastHolsteredState then
			angAdd = self.HolsterAngleAdd * progress
		else
			angAdd = self.HolsterAngleAdd * (1 - progress)
		end
	end
	...
	return eyePos, eyeAngles + Angle(20 * frac, 0, 0) + angAdd
end
