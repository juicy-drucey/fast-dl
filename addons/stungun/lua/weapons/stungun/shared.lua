...
local instructions = {"Left click to stun a person."}
if STUNGUN.CanUntaze then
	table.insert(instructions, "Right click to unstun a person.")
end
if STUNGUN.CanHolster then
	table.insert(instructions, "Reload to holster/unholster the stun gun.")
end

SWEP.Author = "Donkie"
SWEP.Instructions = table.concat(instructions, "\n")
SWEP.Contact = ""
SWEP.Purpose = ""
...
SWEP.HolsterTime = 0.5 -- How many seconds it takes to holster/unholster the weapon
SWEP.HolsterAngleAdd = Angle(10, 0, 0) -- Angle to add to the viewmodel when holstered
...
function SWEP:Initialize()
	...
	if STUNGUN.CanHolster then
		...
		self.NextHolsterAction = 0

		if CLIENT then
			self.LastHolsteredState = false
		end
	else
...
function SWEP:SetupDataTables()
	self:NetworkVar("Bool", 0, "HolsteredInternal")
end

function SWEP:SetHolstered(holstered)
	self:SetHolsteredInternal(holstered)
...
function SWEP:IsHolstered()
	return self:GetHolsteredInternal() or false
end
...
function SWEP:PrimaryAttack()
	if STUNGUN.CanHolster and self:IsHolstered() then
		self:SetHolstered(false)
		self:SetNextPrimaryFire(CurTime() + self.HolsterTime)
		return
	end
		...
		if self.ChargeInserted then
			...
			if self:Clip1Custom() >= 1 then
				...
				if not self.AnimReloadingCharge and self:OwnerHasAmmo() then
					...
					self.AnimReloadingCharge = curtime + (115 / animfps)
					self.AnimChargeInserted = curtime + (83 / animfps)
...
function SWEP:Untaze()
	...
	if self.Owner:GetShootPos():DistToSqr(tr.HitPos) > rangeSqr then return false end
	...
	if not hitRag and not hitFrozenPlayer then return false end

	local ply = hitRag and ent.tazeplayer or ent
	local canUnTaze = hook.Run("PlayerCanUnTaze", self.Owner, ply)
	if canUnTaze == false then return false end
...
function SWEP:SecondaryAttack()
	...
	if STUNGUN.CanUntaze then
		if STUNGUN.CanHolster and self:IsHolstered() then
			self:SetHolstered(false)
			self:SetNextSecondaryFire(CurTime() + self.HolsterTime)
			return
		end
...
function SWEP:ReloadCartridge()
	...
	self.AnimChargeRemoved = CurTime() + (22 / animfps)
	self:SetCartridgeVisible(true)
	...
	if self:OwnerHasAmmo() then
		...
		self.AnimReloadingCharge = CurTime() + (130 / animfps)
		self.AnimChargeInserted = CurTime() + (98 / animfps)
	else
		self:SendWeaponAnim(ACT_VM_HOLSTER)
		self.AnimUnloadingCharge = CurTime() + (100 / animfps)
	end
end

-- No default reloading, we handle that ourselves
function SWEP:Reload()
	if STUNGUN.CanHolster and CurTime() > self.NextHolsterAction then
		self:SetHolstered(not self:IsHolstered())
		self.NextHolsterAction = CurTime() + self.HolsterTime
	end
