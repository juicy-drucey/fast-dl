...
function PANEL:_Lang( )
    self.l_ent_amt_name     = ln( 'mem_energy_name' )
...
function PANEL:Init( )
    ...
    */
    ...
    table.sort( cats, function( a, b )
        local aCat          = a.category and a.category:lower( ) or self:GetCatOther( )
        local bCat          = b.category and b.category:lower( ) or self:GetCatOther( )

        local aSort         = self.cats_src[ aCat ] and self.cats_src[ aCat ].sortOrder or 999
        local bSort         = self.cats_src[ bCat ] and self.cats_src[ bCat ].sortOrder or 999

        local aso           = aSort or a.sortOrder or 100
        local bso           = bSort or b.sortOrder or 100
    ...
    */
    ...
    for v in helper.get.data( cats ) do
        ...
        */
        ...
        local nObj_n                = v.category and self:CreateCat( v.category ) or self:CreateCat( cfg.food.categories[ 'other' ].name )
        ...
        */
        ...
        local ent_energy                            = v.energy and v.energy or 0
            ...
            */
            ...
            :setupanim              ( 'OnHoverFade', 4, rlib.i.OnHover      )
                                    ...
                                    */
                                    ...
                                    :omc( function( s )
                                        if handle:GetAccessSAdmin( 'arivia_sa_tools' ) then
                                            local name          = ( v.name and helper.str:clean( v.name ) ) or ( v.ent and helper.str:clean( v.ent ) ) or ''
...
function PANEL:GetCatData( cat )
    if not isstring( cat ) then return end

    return self.cats_src[ cat:lower( ) ]
end

/*
*	pnl > get category > other
*/

function PANEL:GetCatOther( )
    return ( cfg.food.categories[ 'other' ] and cfg.food.categories[ 'other' ].name ) or  ln( 'mem_cat_other' )
end
...
function PANEL:CreateCat( name )
    ...
    */
    ...
    local clr_cat                   = ( self.cf_i.clrs.bCatColors and self:GetCatData( name ) and self:GetCatData( name ).color ) or self.cf_i.clrs.list.cat_header_gra
    local bExpand                   = self:GetCatData( name ) and self:GetCatData( name ).startExpanded and true or false
    local tip                       = self:GetCatData( name ) and self:GetCatData( name ).tip or nil
...
function PANEL:Populate( v )
    ...
    */
    ...
    local m_sel_fov                 = ( cfg.models.sel.man[ m_sel_src ] and cfg.models.sel.man[ m_sel_src ].fov ) or 105
    ...
    local mdl_oset                  = ( cfg.models.sel.man[ m_sel_src ] and cfg.models.sel.man[ m_sel_src ].oset ) or Vector( 0, 0, -2 )
...
function PANEL:CreateIndicators( )
    ...
    */
    ...
    local clr_ico_nocook            = self.cf_g.sel.clrs.indic_ico_nocook
    ...
    */
    ...
    self.sel.b_is_cook              = ui.new( 'btn', self.sel.indic         )
    :bsetup                         (                                       )
    :left                           ( 'm' , 0, 0, 4, 0                      )
    :sz                             ( 31                                    )
    :tip                            ( ln( 'sel_tip_cook' )                  )
    :enabled                        ( not handle:CanBuyFood( self.item ) and true or false )
                                    ...
                                    :draw( function( s, w, h )
                                        ...
                                        draw.SimpleText( self.cf_u.right.ico.nocook, pref( 'g_ico_hint' ), w / 2, h / 2, ColorAlpha( clr_ico_nocook, a ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                                    end )
...
function PANEL:GetDesc( item )
    ...
    if item.energy then
        local str                   = ln( 'sel_desc_food_amt', item.energy )
        desc                        = desc .. str .. '\n\n'
    end
...
function PANEL:GetAction( item )
    ...
    rcc.run.gmod( 'darkrp', 'buyfood', item.name )
end
...
function PANEL:_Declare( )
    ...
    */
    ...
    self.srctbl                     = mod.food.src
    self.item                       = nil
    self.cats                       = { }
    self.cats_i                     = 0
    self.cats_src                   = cfg.food.categories
...
ui:create( mod, 'pnl_tab_food', PANEL, 'pnl' )