    ...
    */
    ...
    for k, v in helper.get.table( params, SortedPairs ) do
        ...
        */
        ...
        if ( k == 'getMax' ) and isfunction( v ) then
            max         = isnumber( max ) and max or 0
            local func  = v( LocalPlayer( ), max )
            v = func
        end
        ...
        */
        ...
        if ( k == 'getPrice' ) then continue end

        /*
        *   val > getMax
        */

        if ( k == 'CustomCheckFailMsg' ) and not helper.str:ok( v ) then
            v = ln( 'sel_tab_debug_item_none' )
        end
...
ui:create( mod, 'elm_sel_tab_dev_item', PANEL, 'pnl' )