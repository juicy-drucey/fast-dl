...
function zgo2.Multitool.BuyItem(category,itemid,ItemData,OnClose)
	...
	zclib.PointerSystem.Start(ply, function()
		...
		// MainLogic
		local IsPlaceTarget,SnapToEntity = zgo2.Multitool.IsPlaceTarget(ply,ItemData.class,zclib.PointerSystem.Data.Pos,zclib.PointerSystem.Data.HitEntity,category,itemid)
		...
		if IsValid(zclib.PointerSystem.Data.PreviewModel) then
			...
			if zgo2.Multitool.IsHitTarget(ent) and IsPlaceTarget then
				if SnapToEntity then
					...
					local ang = (ply:GetPos() - zclib.PointerSystem.Data.Pos):Angle()
					...
					zclib.PointerSystem.Data.PreviewModel:SetModelScale(1)
				else
