...
zclib.RenderData.Add("zgo2_seedlibary", {
	ang = Angle(0, 180, 0),
	FOV = 10,
})
...
*/
zclib.RenderData.Add("zgo2_seed", {
	ang = Angle(0, 90, -90),
	CamPosOverwrite = Vector(-20, 0,0 ),
	EyeAngOverwrite = Angle(0, 0, 0),
	FOV = 30
})
if CLIENT then

	zclib.Snapshoter.SetPath("zgo2_seed",function(ItemData)
		if ItemData.PlantID then
			return "zgo2/seed_" .. ItemData.PlantID
		end
	end)

	zclib.Hook.Add("zclib_PreRenderStartProductImage", "zclib_PreRenderStartProductImage_ZerosGrowOP2_Seed", function(ItemData)
		if ItemData and ItemData.class == "zgo2_seed" and ItemData.PlantID then
			zgo2.Seed.RebuildMaterial(zgo2.Plant.GetData(ItemData.PlantID), false)
		end
	end)
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- f69c95600bf898b66d56b7a9e25fb8a12eebe9e851363b0f35df32e1d4d4724b

	zclib.Hook.Add("zclib_RenderProductImage", "zclib_RenderProductImage_ZerosGrowOP2_Seed", function(ent, ItemData)
		if ItemData and ItemData.class == "zgo2_seed" and ItemData.PlantID then
			local PlantData = zgo2.Plant.GetData(ItemData.PlantID)
			ent:SetSubMaterial(0, "!zgo2_seedbox_" .. PlantData.uniqueid)
			ent:SetSubMaterial(2, "!zgo2_plant_bud_dried_" .. PlantData.uniqueid)
		end
	end)
end
