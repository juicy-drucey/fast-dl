...
*/
function zgo2.Marketplace.DrawPlane(TransferID,data,pnl)

	local MarketplaceA = data.StartID
	local MarketplaceB = data.DestinationID
	local TravelTime = data.TravelDuration
	local DepatureTime = data.TravelStart
	local MuleID = data.MuleID
	...
	if TransferData.PlanePos then
		...
		if PlayerAvatar and PlayerName then

			local x,y = pnl:CursorPos()
			local range = 50

			local PlaneX,PlaneY = TransferData.PlanePos.x * zclib.wM, TransferData.PlanePos.y * zclib.hM

			if x > (PlaneX - range) and x < (PlaneX + range) and y > (PlaneY - range) and y < (PlaneY + range) then

				surface.SetDrawColor(color_black)
				surface.SetMaterial(zclib.Materials.Get("radial_shadow"))
				surface.DrawTexturedRectRotated(PlaneX,PlaneY, 200 * zclib.wM,200 * zclib.hM, 0)

				draw.SimpleTextOutlined(PlayerName, zclib.GetFont("zclib_font_small"), PlaneX, PlaneY - 30 * zclib.hM, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)

				zclib.BMASKS.BeginMask("zclib_Circle")
				surface.SetDrawColor(color_white)
				surface.SetMaterial(PlayerAvatar)
				surface.DrawTexturedRectRotated(PlaneX, PlaneY, 60 * zclib.wM, 60 * zclib.hM, 0)
				zclib.BMASKS.EndMask("zclib_Circle", PlaneX, PlaneY, 60 * zclib.wM, 60 * zclib.hM, 255, 0)
			end
		end
	end
end
...
/*
	Returns the amount of active transfers initiated by the specified player
*/
function zgo2.Marketplace.GetFlightCount(ply)
	local id64 = ply:SteamID64()
	local count = 0
	for k,v in pairs(zgo2.Marketplace.Transfers) do
		if v and v.Player64 == id64 then
			count = count + 1
		end
	end
	return count
